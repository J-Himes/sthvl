...
#    limits = int(n_frames * proportion)
    # Ensures that the knapsack can hold at least one item
    limits = max(max(nfps), int(n_frames * proportion))
    # For MSRVTT, packed is empty, get key shot based summary never executed
    packed = knapsack(values = seg_scores, weights = nfps, capacity = limits)
    ...
    print(sum(summary), len(summary), proportion, sum(summary)/len(summary))
    print(cps[:5])
    print(nfps)
    print("End of pred_sum")
    return summary
...
def bbox2summary(seq_len: int,
    ...
    pred_summ = get_keyshot_summ(score, change_points, n_frames, nfps, picks, proportion=proportion)
    return pred_summ
